 ...
 *               Revisi/Ditolak/On-time%) dihitung TERPISAH, TIDAK dibaur ke Point
...
class LeaderboardService
{
     ...
     * @return array<int, array{id:int, name:string, point:int, rating:?float, revisi:int, ditolak:int, on_time_percent:?float}>
     */
    public function forPeriod(Collection $users, Carbon $from, Carbon $to): array
    {
        ...
        foreach ($userIds as $id) {
            $acc[$id] = ['point' => 0, 'rating_sum' => 0, 'rating_count' => 0, 'revisi' => 0, 'ditolak' => 0, 'approved_total' => 0, 'on_time' => 0];
        }
